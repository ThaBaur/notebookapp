...
public class RemoteTextProcessorMulti implements ITextProcessor {
	...
	private List<WebTarget> targets =  new LinkedList<WebTarget>();
	AtomicInteger ai = new AtomicInteger();

	public RemoteTextProcessorMulti(List<String> textProcessorResources, Client client) {
		super();
		for (String processor: textProcessorResources) {
			this.targets.add(client.target(processor));
		}
	}
	...
	public String process(String text) {
		...
		String processedText = targets.get(next).request(MediaType.TEXT_PLAIN).post(Entity.entity(text, MediaType.TEXT_PLAIN), String.class);
