		...
		try {
			...
			switch (conf.mom) {
				...
				default:
					...
					// credential provider.
					factory = SQSConnectionFactory.builder().withRegion(Region.getRegion(Regions.US_EAST_1))
			...
			switch (conf.mom) {
				case ActiveMQ:
					// lookup queue
					requestQueue = (Queue) context.lookup(conf.requestQueueName);
					responseQueue = (Queue) context.lookup(conf.responseQueueName);
					break;
				case SQS:
				default:
